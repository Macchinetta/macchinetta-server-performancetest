...
<c:if test="${passengerListSize > 3 }">
  <c:forEach var="passenger" items="${ticketReserveForm.reservationForm.passengerFormList}"
    begin="3" varStatus="status">
    <c:choose>
      <c:when
        test="${!empty passenger.familyName || !empty passenger.givenName || !empty passenger.age || !empty passenger.gender || !empty passenger.customerNo}">
...
<div class="flow">
  <img src="${pageContext.request.contextPath}/resources/image/flow_3_input.jpg"
    alt="[情報入力]">
</div>

<h1>お客様情報入力</h1>
<p class="header_guide">お客様情報を入力し、[次へ]を押してください。お申し込み内容確認画面へ進みます。</p>
...
<form:form name="ticketReserveForm" method="post"
  action="${pageContext.request.contextPath}/Ticket/reserve" modelAttribute="ticketReserveForm">
  ...
  <div id="flight">
    <table>
      ...
      <c:forEach var="reserveFlightBean" items="${selectFlightDtoList}"
        ...
        <tr>
          <td class="info-center">${f:h(reserveFlightBean.lineType.name)}</td>
          <td class="info-center"><fmt:formatDate value="${reserveFlightBean.departureDate}"
              pattern="M月d日(E)" /></td>
          <td class="info-center">${f:h(reserveFlightBean.flightName)}</td>
          <td class="info-center"><fw:split str="${reserveFlightBean.departureTime}" split=":"
              position="2" /></td>
          <td class="info-center"><fw:split str="${reserveFlightBean.arrivalTime}" split=":"
              position="2" /></td>
          <td class="info-center">${f:h(CL_AIRPORT[reserveFlightBean.depAirportCd])}&nbsp;⇒&nbsp;${f:h(CL_AIRPORT[reserveFlightBean.arrAirportCd])}</td>
          <td class="info-center">${f:h(CL_BOARDINGCLASS[reserveFlightBean.boardingClassCd.code])}</td>
          <td class="info-center">${f:h(CL_FARETYPE[reserveFlightBean.fareTypeCd.code])}</td>
          <td class="info-center">&yen;<fmt:formatNumber value="${reserveFlightBean.fare}"
              pattern="###,###" />
          </td>
  ...
  <h2>お客様情報</h2>
  <p class="guide">ご搭乗されるお客様の情報を人数分ご入力ください。</p>
  ...
  <div id="passengerFormList">
    <table id="passengerFormListtable">
      ...
      <c:forEach var="passenger" items="${ticketReserveForm.reservationForm.passengerFormList}"
        begin="0" end="${showPassengerNum-1}" varStatus="passengerListRowStatus">
        <spring:nestedPath path="reservationForm.passengerFormList[${passengerListRowStatus.index}]">
          <tr>
            <td class="center">${f:h(passengerListRowStatus.count)}</td>
            <td>
              <p>
                姓&nbsp;
                <form:input path="familyName" size="15" />
                名&nbsp;
                <form:input path="givenName" size="15" />
              </p> <form:errors path="familyName" cssClass="error" element="p" /> <form:errors
                path="givenName" cssClass="error" element="p" />
            </td>
            <td>
              <p>
                <form:input path="age" size="3" />
                &nbsp;歳
              </p> <form:errors path="age" cssClass="error" element="p" />
            </td>
            <td>
              <p>
                <form:radiobuttons path="gender" items="${CL_GENDER}" cssClass="radio_btn" />
              </p> <form:errors path="gender" cssClass="error" element="p" />
            </td>
            <td>
              <p>
                <form:input path="customerNo" size="15" />
                (任意)
              </p> <form:errors path="customerNo" cssClass="error" element="p" />
            </td>
          </tr>
        </spring:nestedPath>
      </c:forEach>
      <c:forEach begin="${passengerListSize}" end="${showPassengerNum - 1}"
        varStatus="passengerListRowStatus">
        <spring:nestedPath path="reservationForm.passengerFormList[${passengerListRowStatus.index}]">
          <tr>
            <td class="center">${passengerListRowStatus.index + 1}</td>
            <td>姓&nbsp;<form:input path="familyName" size="15" /> 名&nbsp;<form:input
                path="givenName" size="15" />
            </td>
            <td><form:input path="age" size="3" />&nbsp;歳</td>
            <td><form:radiobuttons path="gender" items="${CL_GENDER}" cssClass="radio_btn" /></td>
            <td><form:input path="customerNo" size="15" />(任意)</td>
          </tr>
        </spring:nestedPath>
      </c:forEach>
    </table>
    ...
    <c:if test="${showPassengerNum == 3}">
      <div>
        <form:button type="button" id="addPassengerBtn" class="backward"
          onclick="reserveForm.addPassengerForm();">搭乗者追加</form:button>
      </div>
    </c:if>
  ...
  <h2>予約代表者情報</h2>
  <p class="guide">この予約の代表者となる方の情報をご入力ください。</p>
  ...
  <div id="representativeForm">
    <spring:nestedPath path="reservationForm">
      <table id="representative">
        ...
        <tr>
          <security:authorize access="!hasRole('MEMBER')">
            <td>
              <p>
                姓&nbsp;
                <form:input path="repFamilyName" size="15" />
                名&nbsp;
                <form:input path="repGivenName" size="15" />
              </p> <form:errors path="repFamilyName" cssClass="error" element="p" /> <form:errors
                path="repGivenName" cssClass="error" element="p" />
            </td>
            <td>
              <p>
                <form:input path="repAge" size="3" />
                &nbsp;歳
              </p> <form:errors path="repAge" cssClass="error" element="p" />
            </td>
            <td>
              <p>
                <form:radiobuttons path="repGender" items="${CL_GENDER}" cssClass="radio_btn" />
              </p> <form:errors path="repGender" cssClass="error" element="p" />
            </td>
            <td>
              <p>
                <form:input path="repCustomerNo" size="15" />
                (任意)
              </p> <form:errors path="repCustomerNo" cssClass="error" element="p" />
            </td>
          </security:authorize>
          <security:authorize access="hasRole('MEMBER')">
            <td>${f:h(ticketReserveForm.reservationForm.repFamilyName)}&nbsp;${f:h(ticketReserveForm.reservationForm.repGivenName)}</td>
            <td>${f:h(ticketReserveForm.reservationForm.repAge)}&nbsp;歳</td>
            <td>${f:h(CL_GENDER[ticketReserveForm.reservationForm.repGender.code]) }</td>
            <td>${f:h(ticketReserveForm.reservationForm.repCustomerNo)}</td>
        ...
        <tr>
          <security:authorize access="!hasRole('MEMBER')">
            <td><form:input path="repTel1" size="5" />-<form:input path="repTel2" size="4" />-<form:input
                path="repTel3" size="4" /> <form:errors path="repTel1" cssClass="error" element="p" />
              <form:errors path="repTel2" cssClass="error" element="p" /> <form:errors
                path="repTel3" cssClass="error" element="p" /></td>
            <td colspan="3"><form:input path="repMail" id="rep_mail" /> <form:errors
                path="repMail" cssClass="error" element="p" /></td>
          </security:authorize>
          <security:authorize access="hasRole('MEMBER')">
            <td>${f:h(ticketReserveForm.reservationForm.repTel1)}-${f:h(ticketReserveForm.reservationForm.repTel2)}-${f:h(ticketReserveForm.reservationForm.repTel3)}</td>
            <td colspan="3">${f:h(ticketReserveForm.reservationForm.repMail)}</td>
