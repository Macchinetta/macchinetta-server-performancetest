...
<div id="vacantsearch">
  <form name="searchForm" method="get" action="${pageContext.request.contextPath}/Ticket/search">
    ...
    <spring:nestedPath path="ticketSearchForm">
      <table>
        ...
        <tr>
          ...
          <td class="route_td">${f:h(CL_AIRPORT[ticketSearchForm.flightSearchCriteriaForm.depAirportCd])}&nbsp;⇒&nbsp;${f:h(CL_AIRPORT[ticketSearchForm.flightSearchCriteriaForm.arrAirportCd])}<form:button
              type="submit" class="util-btn" onclick="flightSearchResult.changeAirportCd();">⇔逆区間</form:button>
          </td>
          <td class="boarding_class_td">${f:h(CL_BOARDINGCLASS[ticketSearchForm.flightSearchCriteriaForm.boardingClassCd.code])}<form:button
              type="submit" class="util-btn"
              onclick="flightSearchResult.changeBoardingClassCd('${f:hjs(flightSearchResultOutputDto.otherBoardingClassCd.code)}');">⇔${f:hjs(CL_BOARDINGCLASS[flightSearchResultOutputDto.otherBoardingClassCd.code])}</form:button>
          </td>
        </tr>
      </table>
    </spring:nestedPath>
    ...
    <div class="pagination">
      <c:set var="flightVacantInfoPage"
        value="${flightSearchResultOutputDto.ticketSearchResultDto.flightVacantInfoPage}" />
      <c:set var="startPosition"
        value="${(flightVacantInfoPage.number * flightVacantInfoPage.size) + 1}" />
      <c:set var="endPosition" value="${startPosition + flightVacantInfoPage.numberOfElements - 1}" />
      <p>${startPosition}-${endPosition}&nbsp;&nbsp;&nbsp;便を表示（${flightVacantInfoPage.totalElements}便中）</p>

      <t:pagination page="${flightVacantInfoPage}"
        firstLinkText='<img src="${pageContext.request.contextPath}/resources/image/page_first_on.jpg" alt="先頭へ">'
        previousLinkText='<img src="${pageContext.request.contextPath}/resources/image/page_prev_on.jpg" alt="前へ">'
        nextLinkText='<img src="${pageContext.request.contextPath}/resources/image/page_next_on.jpg" alt="次へ">'
        lastLinkText='<img src="${pageContext.request.contextPath}/resources/image/page_last_on.jpg" alt="最終へ">'
        maxDisplayCount="0" disabledHref="javascript:void(0);"
        pathTmpl="javascript:flightSearchResult.goPage({page});" queryTmpl="" />
    </div>
...
<div id="vacantflightlist">
  <form name="selectForm" method="get" action="${pageContext.request.contextPath}/Ticket/search">
    ...
    <spring:nestedPath path="ticketSearchForm">
      <table>
        ...
        <tr>
          ...
          <c:set var="fareTypeCount"
            value="${flightSearchResultOutputDto.ticketSearchResultDto.fareTypeList.size()}" />
          <c:forEach var="faretype"
            items="${flightSearchResultOutputDto.ticketSearchResultDto.fareTypeList}">
            <th class="fare_th"><c:choose>
                ...
                <c:otherwise>
                  <p class="faretype">${f:h(CL_FARETYPE[faretype.code])}</p>
                </c:otherwise>
              </c:choose></th>
          </c:forEach>
        ...
        <%-- 実データ表示 --%>
        <c:forEach var="flightVacantInfo" items="${flightVacantInfoPage.content}"
          varStatus="rowStatus">
          <c:set var="flightMaster" value="${flightVacantInfo.flightMaster}" />
          <tr>
            <th class="label">${f:h(flightMaster.flightName)}</th>
            <th class="label"><fw:split str="${flightMaster.departureTime}" split=":"
                position="2" /></th>
            <th class="label"><fw:split str="${flightMaster.arrivalTime}" split=":"
                position="2" /></th>
            <th class="label">${f:h(flightMaster.plane.craftType)}</th>

            <%-- 運賃種別情報 --%>
            <c:forEach var="fareTypeCd"
              items="${flightSearchResultOutputDto.ticketSearchResultDto.fareTypeList}"
              varStatus="queStatus">
              <c:set var="fareTypeVacantInfo"
                value="${flightVacantInfo.fareTypeVacantInfoMap[fareTypeCd]}" />
              <td><c:set var="vacantNum" value="${f:h(fareTypeVacantInfo.vacantNum)}" /> <c:choose>
                  ...
                  <c:otherwise>
                    <form:button type="button" name="select" class="vacant-seat-0 disabled"
                      disabled="true">×</form:button>
                  </c:otherwise>
                </c:choose> &yen;<fmt:formatNumber value="${fareTypeVacantInfo.fare}" pattern="###,###" /></td>
            </c:forEach>
            <c:forEach begin="${fareTypeCount}" end="7">
              <th class="null"></th>
            </c:forEach>
      ...
      <div id="example">
        <form:button type="button" class="vacant-seat-over20 legend-btn">〇</form:button>
        <span>空席あり(20席以上)</span>
        <form:button type="button" class="vacant-seat-over10 legend-btn">△</form:button>
        <span>空席あり(10～19席)</span>
        <form:button type="button" class="vacant-seat-over0 legend-btn">9</form:button>
        <span>空席あり(1～9席)</span>
        <form:button type="button" class="vacant-seat-0 legend-btn" disabled="true">×</form:button>
        <span>満席</span>
      </div>
    ...
    <fmt:formatDate value="${flightSearchResultOutputDto.depDate}" pattern="yyyy-MM-dd"
      var="formattedDepDateString" />
    <spring:nestedPath path="ticketSearchForm.selectFlightFormList[${selectFlightIndex}]">
      ...
      <form:hidden path="boardingClassCd"
        value="${ticketSearchForm.flightSearchCriteriaForm.boardingClassCd}" />
      <form:hidden path="fareTypeCd" />
      <form:hidden path="flightName" />
    </spring:nestedPath>
...
<div class="navi-backward">
  <form name="backForm" method="get" action="${pageContext.request.contextPath}/Ticket/search">
    ...
    <input type="hidden" name="flightSearchCriteriaForm.month"
      value="${f:h(ticketSearchForm.outwardLineSearchCriteriaForm.month)}" /> <input type="hidden"
      name="flightSearchCriteriaForm.day"
      value="${f:h(ticketSearchForm.outwardLineSearchCriteriaForm.day)}" /> <input type="hidden"
      name="flightSearchCriteriaForm.depAirportCd"
      value="${f:h(ticketSearchForm.outwardLineSearchCriteriaForm.depAirportCd)}" /> <input
      type="hidden" name="flightSearchCriteriaForm.arrAirportCd"
      value="${f:h(ticketSearchForm.outwardLineSearchCriteriaForm.arrAirportCd)}" /> <input
      type="hidden" name="flightSearchCriteriaForm.boardingClassCd"
      value="${f:h(ticketSearchForm.outwardLineSearchCriteriaForm.boardingClassCd)}" /> <input
      type="hidden" name="flightSearchCriteriaForm.time"
      value="${f:h(ticketSearchForm.outwardLineSearchCriteriaForm.time)}" />
  </form>
</div>
